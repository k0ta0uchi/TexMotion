...
csc = r"C:\Program Files\dotnet\sdk\8.0.319\Roslyn\bincore\csc.dll"
...
# Required Editor files for Grounding
editor_files = [
    ...
    r"C:\Workspace\TexMotion\tests\GroundingVerificationTests.cs"
]
for f in editor_files:
    rsp_lines.append(f'"{f}"')

rsp_path = r"C:\Workspace\TexMotion\grounding_test.rsp"
with open(rsp_path, "w", encoding="utf-8") as f:
    f.write("\n".join(rsp_lines))

print("Compiling grounding_test_runner.exe...")
...
if res.returncode != 0:
    ...
    run_res = subprocess.run([mono_exe, "grounding_test_runner.exe"], capture_output=True, env=env)
    ...
    if os.path.exists(rsp_path):
        os.remove(rsp_path)
    if os.path.exists("grounding_test_runner.exe"):
        os.remove("grounding_test_runner.exe")
