...
* **Motion Timeline and Pose Editor**: Provides a modular 4-tab visual timeline workspace (Pose, Repair, Timing, Polish) to inspect and adjust animation clips frame-by-frame. Features a 3D studio stage with contact shadows, background switching (Stage/Dark/Green), per-joint Euler angle and root position editing, pose copying and mirroring, jitter reduction via Slerp interpolation, and variable playback speeds (0.1x to 2.0x).
...
* **モーションタイムラインとポーズ編集**: 「ポーズ」「修復」「タイミング」「ポリッシュ」の 4 大タブにモジュール化された専用タイムラインエディタを備えており、生成したクリップをフレーム単位で調整できます。チェッカー床・動的シャドウ付きの 3D スタジオステージ（Stage/Dark/Green 背景切替対応）、関節ごとのオイラー角やルート移動の編集、ポーズのコピーと反転貼り付け、Slerp 補間によるブレの抑制、0.1 倍から 2.0 倍までの可変速再生に対応します。
