...
* **Advanced Pose Correction Suite**: Features 11 specialized tools to resolve tracking noise and pose degradation: 3D onion skinning (cyan/magenta ghost frames), 2-Bone IK dragging on extremities, In/Out range selection with body part masking (Upper, Lower, Arms, Legs), easing tweens, loop boundary blending, additive range offsets, armpit/chest penetration limiters, foot grounding and sliding locks, glitch detection with automated Slerp repair, pose palette with 8 save slots, and range retiming. (See the [Advanced Pose Correction Guide](docs/timeline_editor_advanced_guide.md) for detailed tutorials and specifications).
...
* **高度ポーズ修正ツール群**: 姿勢推定時のノイズやトラッキング破綻を修正するための 11 種類のツールを統合しています。3D オニオンスキン表示（前後フレームの色分けゴースト描画）、四肢先端の 2-Bone IK ドラッグ操作、In/Out 区間指定と部位別マスク（上半身、下半身、腕、脚）、イージング補間（トゥイーン）、ループ境界ブレンダー、アディティブ範囲オフセット、脇・胸部めり込み防止リミッター、地面接地スナップおよび足滑り固定、異常フレームの自動検出と一括修復、8 スロットのポーズパレット、および区間リタイミングに対応します（詳細は [高度ポーズ修正ガイド](docs/timeline_editor_advanced_guide.md) を参照）。
