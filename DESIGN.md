...
## TexMotion EditorWindow Application

`TexMotionWindow` is a legacy IMGUI surface, so the tokens above map to cached dark
cards, hairline separators, compact spacing, and restrained accent use. The editor
keeps navigation, avatar/video source selection, Target FPS, and primary extraction
or download actions visible; infrequently used controls move behind compact foldout
headers with a short status summary.

- Video Motion: `Advanced Extraction` contains trimming and processing quality; the
  Pose Estimation Backend dropdown remains visible as a primary extraction control,
  while `Hand & Face Assistance` is a separate optional section.
- Settings: model repository, local storage, Video Motion Python Environment,
  Video2Motion Model Assets, Video2Motion Inference, and Workflow are independent
  foldout cards, while Language and the main model download action remain reachable.
- Foldout state is UI-only and persisted with `EditorPrefs`. Backend values continue
  to live in `TexMotionSettings`, so collapsing a section does not alter extraction
  behavior or model selection.

### Motion Timeline Editor

The Timeline Editor uses the same token contract as TexMotion Studio. The window
canvas is Void, the top navigation and timeline are Carbon, and viewport/inspector
cards step up to Obsidian with Graphite hairline boundaries. The header is split into
an identity/action row and a playback row so clip metadata, Save, Apply, transport,
undo, and revert actions remain scannable at the minimum window size.

- **Primary action:** `Apply to Avatar` is the single Acid Lime action in the view.
  Save, playback, editing, and navigation controls use neutral Ghost surfaces; the
  play state uses a restrained Pulse Green tint.
- **Workspace hierarchy:** the viewport, pose inspector, and bottom timeline are
  independent cards with 8px gaps. The inspector keeps secondary pose tools behind a
  collapsed foldout while frame navigation remains at the top of the card.
- **Timeline semantics:** the Acid Lime playhead and current frame, Pulse Green
  modified frames, and Coral Red uncertainty spans are the only chromatic timeline
  signals. Ruler/grid lines stay Graphite/Fog so pose data remains the focal point.
- **Empty state:** an Obsidian guide card explains how to load a clip and presents a
  single Acid Lime link back to TexMotion Studio. All copy uses the shared localization
  catalog and cached IMGUI styles; no per-frame GUIStyle or texture allocation is
  permitted.
